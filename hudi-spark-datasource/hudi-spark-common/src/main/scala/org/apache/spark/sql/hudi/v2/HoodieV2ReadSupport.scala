...
/**
 * Supportability gate and options resolution for the DSv2 read path.
 *
 * The DSv2 scan is base-file-only, Parquet-only, single-format, and does not implement
 * incremental / CDC / bootstrap. Time travel (`as.of.instant`) is deliberately supported.
 */
