...
/**
 * Batch scan for CDC queries via DSv2.
 */
class HoodieCdcBatchScan(cdcReadSchema: StructType,
  ...
  override def description(): String = s"HoodieCdcBatchScan${cdcReadSchema.catalogString}"
  ...
  override def createReaderFactory(): PartitionReaderFactory = {
    new HoodieCdcPartitionReaderFactory(
      broadcastReader,
      broadcastConf,
      basePath,
      originTableSchema,
