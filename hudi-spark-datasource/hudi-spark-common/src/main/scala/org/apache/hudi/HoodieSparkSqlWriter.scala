...
import org.apache.hudi.dsv2.HoodieDatasourceV2Writer
      ...
      val (writeResult: HoodieWriteResult, writeClient: SparkRDDWriteClient[_]) =
        operation match {
          ...
          case _ =>
            ...
            // Short-circuit if Spark DataSource V2 write is enabled — the dsv2 path runs
            // its own commit + meta-sync flow, independent of the bulk-insert row writer.
            // scalastyle:off return
            if (hoodieConfig.getBoolean(DATASOURCE_V2_WRITE_ENABLED)) {
              return HoodieDatasourceV2Writer.write(
                sqlContext, client, df, parameters, hoodieConfig, writerSchema, tableConfig, basePath,
                () => metaSync(sparkSession, hoodieConfig, basePath, df.schema))
            }
