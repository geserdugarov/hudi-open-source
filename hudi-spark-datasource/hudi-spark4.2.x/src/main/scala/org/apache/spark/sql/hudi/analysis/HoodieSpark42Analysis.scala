...
/**
 * Converts Hudi DSv2 relations back to V1 [[LogicalRelation]]s:
 *
 * - [[HoodieInternalV2Table]] (schema-on-read path, HUDI-4178) always falls back since it
 *   does not implement the DSv2 read API;
 * - [[HoodieSparkV2Table]] (`hoodie.datasource.read.use.v2`) keeps SELECTs on the DSv2 read
 *   path, but INSERT/UPDATE/DELETE/MERGE targets are converted so the Hudi SQL command
 *   rewrites and their V1 input scans keep working.
 */
case class HoodieSpark42DataSourceV2ToV1Fallback(sparkSession: SparkSession) extends Rule[LogicalPlan]
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan match {
    ...
    case iis@InsertIntoStatement(rv2@DataSourceV2Relation(v2Table: HoodieSparkV2Table, _, _, _, _, _), _, _, _, _, _, _, _, _)
      if v2Table.hoodieCatalogTable.isDefined =>
      iis.copy(table = convertToV1(rv2, v2Table))

    case _ =>
      // HoodieSparkV2Table SELECTs must stay on the DSv2 read path, so bare relations are
      // left untouched. DML statements, however, scan their target through the relation
      // plan embedded in the rewritten Hudi commands (e.g. UpdateHoodieTableCommand), so
      // Hudi V2 relations under UPDATE/DELETE/MERGE are converted back to V1 to keep
      // those scans on the V1 path.
      val isDmlStatement = plan.isInstanceOf[UpdateTable] ||
        plan.isInstanceOf[DeleteFromTable] || plan.isInstanceOf[MergeIntoTable]
      plan.resolveOperatorsDown {
        case rv2@DataSourceV2Relation(v2Table: HoodieInternalV2Table, _, _, _, _, _) => convertToV1(rv2, v2Table)
        case rv2@DataSourceV2Relation(v2Table: HoodieSparkV2Table, _, _, _, _, _)
          if isDmlStatement && v2Table.hoodieCatalogTable.isDefined => convertToV1(rv2, v2Table)
      }
  }
