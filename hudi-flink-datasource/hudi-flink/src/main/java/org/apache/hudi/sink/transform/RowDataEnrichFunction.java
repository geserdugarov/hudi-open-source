...
/**
 * Function that adds key and partition to RowData
 */
public class RowDataEnrichFunction<I extends RowData, O extends Tuple>
  ...
  private Tuple3<BinaryStringData, BinaryStringData, RowData> enrichRowData(I record) {
    GenericRecord gr = (GenericRecord) this.converter.convert(this.avroSchema, record);
    final HoodieKey hoodieKey = keyGenerator.getKey(gr);
    // added key and partition in tuple (recordKey, partitionPath, rowData)
    return new Tuple3<>(new BinaryStringData(hoodieKey.getRecordKey()), new BinaryStringData(hoodieKey.getPartitionPath()), record);
  }
}
