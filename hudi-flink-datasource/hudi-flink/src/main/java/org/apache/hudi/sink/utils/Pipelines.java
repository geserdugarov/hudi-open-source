...
public class Pipelines {
  ...
  public static DataStream<HoodieFlinkInternalRow> bootstrap(
      ...
      DataStream<RowData> dataStream) {
    return bootstrap(conf, rowType, rowDataInfo, dataStream, false, false);
  }
  ...
  public static DataStream<HoodieFlinkInternalRow> bootstrap(
    ...
    if (overwrite || OptionsResolver.isBucketIndexType(conf)) {
      return rowDataToHoodieRecord(conf, rowType, rowDataInfo, dataStream);
    } else if (bounded && !globalIndex && OptionsResolver.isPartitionedTable(conf)) {
      return boundedBootstrap(conf, rowType, rowDataInfo, dataStream);
    } else {
      return streamBootstrap(conf, rowType, rowDataInfo, dataStream, bounded);
    }
  }
  ...
  private static DataStream<HoodieFlinkInternalRow> streamBootstrap(
      ...
      DataStream<RowData> dataStream,
      boolean bounded) {
    DataStream<HoodieFlinkInternalRow> dataStream1 = rowDataToHoodieRecord(conf, rowType, rowDataInfo, dataStream);
  ...
  public static DataStream<HoodieFlinkInternalRow> rowDataToHoodieRecord(Configuration conf,
                                                                         ...
                                                                         DataStream<RowData> dataStream) {
    return dataStream
        .map(RowDataToHoodieFunctions.create(rowType, conf), new HoodieFlinkInternalRowTypeInfo(rowType, rowDataInfo))
