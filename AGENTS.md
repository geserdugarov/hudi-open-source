...
# Apache Hudi — Agent Instructions

This file provides repository-specific guidance for automated agents
(Claude Code, Codex, and others) working with code in this repository.
...
Check if variable `MAVEN_REPO_PATH` was set in `./.env` file (do not use `./.env-example`).
If not found then check session environment for presence of `MAVEN_REPO_PATH`.
If not found then ask for local Maven repository path, and save it to `MAVEN_REPO_PATH` variable.
Always add `-Dmaven.repo.local=${MAVEN_REPO_PATH}` to all Maven goal runs.
...
Check if variable `JAVA_HOME_PATH` was set in `./.env` file (do not use `./.env-example`).
If not found then check `sdk list java | grep 'installed\|local'`, search for needed version among them,
and set `JAVA_HOME_PATH` correspondingly.
If not found then try to search for the needed version some other way, and set `JAVA_HOME_PATH`
Always add `JAVA_HOME=${JAVA_HOME_PATH}` before all Maven goal runs, for instance, `JAVA_HOME=${JAVA_HOME_PATH} mvn compile`.

To prevent issues with SSL certificates always add
`-Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true`
to all Maven goal runs.

Also add `-Dmaven.wagon.httpconnectionManager.ttlSeconds=25 -Dmaven.wagon.http.retryHandler.count=5`
to avoid stale connections.

Use Spark 3.5 and Flink 1.20 by default, to do so add `-Dspark3.5 -Dflink1.20`.
...
- Commit subjects follow Conventional Commits with an optional scope: `<type>(<scope>): <subject>`
  (e.g. `feat(flink): ...`, `fix: ...`, `perf(common): ...`, `docs(spark): ...`, `chore(common): ...`).
