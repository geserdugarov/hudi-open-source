...
- Changed from `data.write.format("org.apache.hudi").save()` to calling `HoodieSparkSqlWriter.write()` directly with `schemaFromCatalog` for proper schema reconciliation with the full table schema (including meta-fields).
